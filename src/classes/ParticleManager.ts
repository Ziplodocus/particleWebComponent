...
export class ParticleManager extends EventEmitter {
  ...
  */
  handleNearbyParticle(p: Particle, q: Particle) {
    ...
    if (inVicinity) this.trigger('inVicinity', { p, q });
  ...
  setGrid() {
    ...
    this.grid = this.grid.map(
      (_) => new Array(Math.ceil(this.bounds.y / this.cellSize))
    );
    this.particles.forEach((p) => {
